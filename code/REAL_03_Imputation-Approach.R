"
Script to analyse the real multi-omics data set with the imputation approach
"
...
get_predicition   <- function(train, test, ntree = 300, mtry = NULL, min_node_size = 5) {
  ...
  # 1-1 Remove the Features from Train that were not observed in Test
  observed_test_cols     <- sapply(1:ncol(test), 
                                   FUN = function(x) !is.na(test[1,x]))
  observedtest_col_names <- colnames(test)[observed_test_cols]
  curr_train_data        <- train[,observedtest_col_names]
...
for (i in 1:5) {
  ...
  # --3 Imputation of the missing values in the trainset!
  data_imputed <- missForest(train_x, verbose = TRUE, parallelize = "forests", 
                             maxiter = maxiter, ntree = ntree_imp)
  
  # --3-1 Bind the imputed data and the response toa single DF
  train_data <- cbind("outcome_Y" = train_y, data_imputed$ximp)
  
  # --4 Create Predicitions for the test-set
  # --4-1 Find the observations with the same observed feature-space
  # --4-1-1 Bind test-features and their response 'Y'
  test_data <- cbind("outcome_Y" = test_y, test_x)
  
  # --4-1-2 get the observed feas per obs.
  observed_feas_test <- foreach(x = seq_len(nrow(test_data)), .combine = 'c') %dopar% {
    paste0(which(!(is.na(test_data[x,]))), collapse = "_")
  }
  
  # --4-1-3 Keep the unique observed test feas [equals the different folds]
  observed_test_folds <- unique(observed_feas_test)
  print(paste0("Found ", length(observed_test_folds), " unique test folds!"))

  # --4-2 For each the 'observed_test_folds' train a RF and create preds
  # --4-2-1 Vector to save the results
  predicted    <- c()
  true_reponse <- c()
  for (curr_test in 1:length(observed_test_folds)) {
    ...
    # --5 Get the predicitions on the current test_data
    curr_predicted <- get_predicition(train = train_data, test = curr_fold_test_data,
                                      ntree = 300, mtry = NULL, min_node_size = 5)
    
    predicted <- c(predicted, as.character(curr_predicted))
...
save(imp_res_all, file = "./docs/CV_Res/REAL/Imputation_Approach.R")