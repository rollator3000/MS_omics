...
cancertypeshort <- "SARC"
cancertypelong <- "Sarcoma_SARC"
...
logmod <- glm(race ~ gender, family=binomial(link='logit'), data=clin2train)

racepred <- as.numeric(predict(logmod, newdata=data.frame(gender=clin2test$gender), type="response") > 0.5)
