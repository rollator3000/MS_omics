...
cancertypeshort <- "OV"
cancertypelong <- "Ovarian_Serous_Cystadenocarcinoma_OV"
...
library("DMwR")
try(knnImputation(clin, k = 10))
# --> For 'knnyImputation()' to work, there have to be at least two columns in the data frame.
# --> Use logistic regression instead:

clin2 <- clin
clin2$race <- as.numeric(as.character(clin$race))
clin2train <- clin2[!is.na(clin2$race),]
clin2test <- clin2[is.na(clin2$race),]

logmod <- glm(race ~ age, family=binomial(link='logit'), data=clin2train)

racepred <- as.numeric(predict(logmod, newdata=data.frame(age=clin2test$age), type="response") > 0.5)
clin2$race[is.na(clin2$race)] <- racepred

clin2$race <- factor(clin2$race, levels=c(1,0))
