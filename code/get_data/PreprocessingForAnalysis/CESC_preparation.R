...
cancertypeshort <- "CESC"
cancertypelong <- "Cervical_Squamous_Cell_Carcinoma_and_Endocervical_Adenocarcinoma_CESC"
...
# --> Only female patients. --> Do not include 'gender'.
# 
# clin$gender <- 0
...
# --> No patients for which all measurements are available.


# --> Investigate reason for the latter:

datnames <- c("clin", "mirna", "mutation", "cnv", "rna")
apply(combn(5,2), 2, function(x) eval(parse(text=paste("length(intersect(rownames(", datnames[x[1]], "), rownames(", datnames[x[2]], ")))", sep=""))))
apply(combn(5,2), 2, function(x) c(datnames[x[1]], datnames[x[2]]))

# --> mirna measurements are from other patients than those
# in the data set.
# --> Do not include mirna data.

comids <- intersect(intersect(intersect(rownames(clin), rownames(mutation)), rownames(cnv)), rownames(rna))
length(comids)
...
save(clin, targetvar, mutation, cnv, rna, file=paste("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Data/ProcessedData/", cancertypeshort, ".Rda", sep=""))
