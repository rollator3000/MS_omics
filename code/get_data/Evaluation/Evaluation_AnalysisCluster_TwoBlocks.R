...
load("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Results/scenariogridtwoblocks.Rda")

allinds <- 1:nrow(scenariogrid)

allresults <- list.files("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Results/ResultsTwoBlocks")
indres <- gsub("res", "", allresults)
indres <- sort(as.numeric(gsub(".Rda", "", indres)))

(anymissing <- length(setdiff(allinds, indres)) > 0)
...
for(i in 1:nrow(scenariogrid)) {
  load(paste("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Results/ResultsTwoBlocks/res", indres[count], ".Rda", sep=""))
  Results[[count]] <- res
  count <- count + 1
}
...
# --> The cases for which the computed C index was 'NA' were
# very rare.
...
# Figure: Clinical covariates plus RNA measurements: C index values obtained for the individual repetitions 
...
ggsave("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/Results_AnalysisClusterTwoBlocks_1.pdf", width=6.5*1.35, height=8*1.35)
...
ggsave("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/Results_AnalysisClusterTwoBlocks_2.pdf", width=6.5*1.35, height=8*1.35)
...
# Adjust the p values:
psadj <- p.adjust(p=ps, method = "holm")
psadj




# Comparison of BlockForest and RandomBlock with RSF:

sum(resultswide2[,"cindex.BlockForest"] > resultswide2[,"cindex.RSF"])
mean(resultswide2[,"cindex.BlockForest"] > resultswide2[,"cindex.RSF"])

sum(resultswide2[,"cindex.RandomBlock"] > resultswide2[,"cindex.RSF"])
mean(resultswide2[,"cindex.RandomBlock"] > resultswide2[,"cindex.RSF"])




# Figure: Clinical covariates plus RNA measurements: Performances of 
# all six considered methods:
...
ggsave(file="./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/Figure3.pdf", plot=pall, width=7.5, height=14)
...
# Figure: Clinical covariates plus RNA measurements: Performances of BlockForest and RandomBlock relative to that of RSF.
...
ggsave("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/Figure4.pdf", width=12*0.85, height=6.5*0.85)
...
# Figures: Clinical covariates plus RNA measurements: tuning parameter values optimized for the different variants:
...
resultsparamvalues$cval_clin <- sapply(paramvaluesnoNA, function(x) x[1])
resultsparamvalues$cval_rna <- sapply(paramvaluesnoNA, function(x) x[2])
...
numbercovariates <- numbercovariates[,c("clin", "rna")]
...
for(i in 1:nrow(numbercovariates)) {
  ...
  restemp <- resultsparamvalues[resultsparamvalues$dat==rownames(numbercovariates)[i] & 
                              resultsparamvalues$method=="VarProb",][,c("cval_clin", "cval_rna")]
...
resultsparamvaluestemp <- reshape(resultsparamvaluestemp, varying=c("cval_clin", "cval_rna"), 
                              v.names="cval", 
                              timevar="block", times=c("clin", "rna"),
                              direction="long")
...
ggsave(paste("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/TunParamTwoBlocks_", methods[i], "_1.pdf", sep=""), width=6*1.4, height=8*1.4)
...
ggsave(paste("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/TunParamTwoBlocks_", methods[i], "_2.pdf", sep=""), width=6*1.4, height=7*1.4)



# The values of the optimized selection probabilities associated with VarProb
# are for all data sets larger for the clinical block than those for the RNA block:
...
resultsparamvaluestempsumclin <- resultsparamvaluestempsum[resultsparamvaluestempsum$block=="clin",]
resultsparamvaluestempsumrna <- resultsparamvaluestempsum[resultsparamvaluestempsum$block=="rna",]

all(resultsparamvaluestempsumclin$cval > resultsparamvaluestempsumrna$cval)
...
# In the median, the block selection probability of the RNA block was
# 3.7 higher than that of the clinical block.

library("plyr")
resultsparamvaluestempsum <- ddply(resultsparamvaluestemp, .variables=c("method", "dat", "block"), .fun=summarise, cval=mean(cval, na.rm=TRUE))

boxplot(sapply(unique(resultsparamvaluestempsum$dat), function(x) resultsparamvaluestempsum$cval[(resultsparamvaluestempsum$block=="rna") & (resultsparamvaluestempsum$dat==x)]/
       resultsparamvaluestempsum$cval[(resultsparamvaluestempsum$block=="clin") & (resultsparamvaluestempsum$dat==x)]))

median(sapply(unique(resultsparamvaluestempsum$dat), function(x) resultsparamvaluestempsum$cval[(resultsparamvaluestempsum$block=="rna") & (resultsparamvaluestempsum$dat==x)]/
              resultsparamvaluestempsum$cval[(resultsparamvaluestempsum$block=="clin") & (resultsparamvaluestempsum$dat==x)]))
...
round(tapply(resultsparamvaluestempsum$cval, resultsparamvaluestempsum$block, mean), 2)
...
# Figure: Clinical covariates plus RNA measurements: Differences between the mean C index values obtained using BlockForest
# and that obtained using RSF plotted against the values of 'n' (left panel), 
# 'b_clin' (middle panel), and 'signal' (right panel).
...
# 
# 
# resultstempsum$maxprob <- apply(resultstempsum[,2:3], 1, max, na.rm=TRUE)
# 
...
# 
# save(resultstempsum, file="./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Results/resultstempsum_twoblocks.Rda")


# Load the result of the out-commented part above:

load("./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Results/resultstempsum_twoblocks.Rda")




x <- resultstempsum$cval_clin
y <- resultstempsum$cindex.BlockForest - resultstempsum$cindex.RSF

pred <- loess.smooth(x, y)
dataloess <- data.frame(x=pred$x, y=pred$y)

p2 <- ggplot(resultstempsum, aes(x=cval_clin, y=cindex.BlockForest - cindex.RSF)) + theme_bw() + geom_point() + 
  geom_hline(yintercept=0, col="grey") +
  geom_line(data=dataloess, aes(x=x, y=y), col="blue") + ggtitle(expression(paste("Influence of '", b["clin"], "'", sep=""))) + labs(x=expression(b["clin"]), y="diffC") +
...
ggsave(file="./Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION/Figures/FactorsDifferenceInPerformanceTwoBlocks.pdf", plot=pall, width=14*0.75, height=5*0.75)
