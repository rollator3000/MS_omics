"Script to download multi-omics data, used in this project!"

# NOTE: 
# Before the code can be excecuted, the working directory MUST be set to the 
# directory that contains the folder
# 'Additional_file_2_HornungWright_WITH_DATA_EXECUTABLE_VERSION':
# (the path 'Z:/here/is/my/path/' *MUST* end with a slash '/'):

# Set 'basedir' to the repository for MS-Thesis
basedir <- "C:/Users/kuche_000/Desktop/MS-Thesis/"
setwd(paste(basedir, "data/external/Dr_Hornung/Data/Download_FirstPreprocessing/", sep = ""))

# Install Packages:
# if (!requireNamespace("BiocManager", quietly = TRUE))
#   install.packages("BiocManager")
# BiocManager::install("SummarizedExperiment")
# BiocManager::install("TCGAbiolinks")                     # one of these should be
# BiocManager::install("BioinformaticsFMRP/TCGAbiolinks")  # working to install the !
#                                                          # 'TCGAbiolinks'
library(TCGAbiolinks)
library(SummarizedExperiment)

#----- RNA expression, measured by RNAseq, raw counts -----#
# Get table with query results (data category/ type, file id, platform, workflow...)
query <- GDCquery(project = "TCGA-BLCA", 
                  data.category = "Transcriptome Profiling",
                  data.type = "Gene Expression Quantification", 
                  workflow.type = "HTSeq - Counts")
# Download the data & create folders!
GDCdownload(query, files.per.chunk = 20) 
# --> Creates empty folders in:
#     "getwd()/TCGA-BLCA\harmonized\Transcriptome_Profiling\Gene_Expression_Quantification"
data <- GDCprepare(query)
rna  <- assay(data) 
save(rna, file = "RNASeq_Counts_BLCA.Rdata")
