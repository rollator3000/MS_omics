...
# ----- ----- ----- Fold-Wise Approach                                        ----
# Combine the results of all FW approaches [pattern1, .., 3] into a single plot!
# [0] Define variables
# 0- 1 Paths to the results of the CV
...
for (pattern_ in c(1, 2, 3, 4)) {
  ...
  for (curr_file in list.files(curr_path)) {
    ...
    curr_df         <- extract_avg_metrics_FW(file_curr, metric = "F1", 
                                              train_sit = pattern_)
...
for (patt_ in c(1, 2, 3, 4)) {
  ...
  # Get the results for the current pattern
  DF_curr <- DF_FW[DF_FW$pattern == patt_,]
